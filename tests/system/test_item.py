...
class ItemTest(BaseTest):
    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                response = client.post('/item/test', data={'price': 10.99, 'store_id': 1})

                self.assertEqual(response.status_code, 201)
                self.assertDictEqual({'name': 'test', 'price': 10.99},
                                     json.loads(response.data))

    def test_create_item_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel('test', 10.99, 1).save_to_db()
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                response = client.put('/item/test', data={'price': 10.99, 'store_id': 1})
